...
define(['N/search', 'N/ui/serverWidget', 'N/log', 'N/task', 'N/redirect', 'N/url', 'N/format', 'N/record', "N/runtime", 'N/currentRecord', 'N/error', 'N/file', '../config/me_config.js'], function (search, serverWidget, log, task, redirect, url, format, record, runtime, currentRecord, error, file, config) {
    ...
    function getTransaaction(account) {
        ...
        let transactionSearchObj = search.create({
            ...
            columns:
                [
                    ...
                    search.createColumn({ name: "custbody_me_transfer_type", label: "ME - Transfer Type" }),
                    search.createColumn({ name: "account", label: "Charges Account/Debited Account" }),
                    search.createColumn({
                        name: "formulatext",
                    ...
                    search.createColumn({
                        name: "formulatext",
                        formula: "case when {customer.custentity_me_rcv_bank_code} is not null then {customer.custentity_me_rcv_bank_code} when {vendor.custentity_me_rcv_bank_code} is not null then {vendor.custentity_me_rcv_bank_code} end",
                        label: "Receiver Bank Code"
                    }),
                    search.createColumn({
                        name: "formulatext",
                        formula: "case when {customer.custentity_me_rcv_bank_name} is not null then {customer.custentity_me_rcv_bank_name} when {vendor.custentity_me_rcv_bank_name} is not null then {vendor.custentity_me_rcv_bank_name} end",
                        label: "Receiver Bank Name"
                    }),
                    search.createColumn({
                        name: "formulatext",
                        formula: "case when {vendor.altname} is not null then {vendor.altname} when {customer.altname} is not null then {customer.altname} end",
                        label: "Name"
                    }),
                    search.createColumn({ name: "internalid", label: "Internal ID" }),
        ...
        for (let i = 0; i < transactionSearchObj.length; i++) {
            ...
            let transfer_type = transactionSearchObj[i].getText(transactionSearchObj[i].columns[1])
            let account = transactionSearchObj[i].getText(transactionSearchObj[i].columns[2])
    ...
    function generateCsv(data, context) {
        ...
        var csvContent = `"Transaction ID";"Transfer Type";"Debited Account";"Beneficiary";"Credited Amount";"Amount";"Effective Date";"Transaction Purpose";"Currency";"Charges Type";"Charges Account";"Remark 1";"Remark 2";"Receiver Bank Code";"Receiver Bank Name";"Receiver Name";"Receiver Cust Type";"Receiver Cust Residence";"Transaction Code";"Beneficiary Email"\n`;



        for (let i = 0; i < data.length; i++) {
            csvContent += `${data[i].transaction_display};${data[i].transfer_type};${data[i].debited_account};${data[i].benificiary_id};${data[i].credited_account};${data[i].amount};${data[i].effective_date};${data[i].transaction_purpose};${data[i].currency};${data[i].charges_type};${data[i].charges_account};${data[i].remark_1};${data[i].remark_2};${data[i].receiver_bank_code};${data[i].receiver_bank_name};${data[i].receiver_name};${data[i].receiver_cust_type};${data[i].receiver_cust_residence};${data[i].transaction_code};${data[i].beneficiary_email}\r\n`;
        }
        ...
        if (context.request.method === 'GET') {
            ...
            if (checked_trans.length > 0) {
                for (let i = 0; i < checked_trans.length; i++) {
                    log.debug("checked_trans[i].transaction_display", checked_trans[i].transaction_display)
                    if (checked_trans[i].transaction_display.includes("Check")) {
                        record.submitFields({
                    ...
                    if (checked_trans[i].transaction_display.includes("Bill Payment")) {
                        record.submitFields({
