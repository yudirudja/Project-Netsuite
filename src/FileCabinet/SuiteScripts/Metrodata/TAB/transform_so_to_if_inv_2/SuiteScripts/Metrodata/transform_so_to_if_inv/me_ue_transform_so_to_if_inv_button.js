...
define(["N/runtime", "N/record", "N/url", "N/search"], function (runtime, record, url, search) {
    ...
    function beforeLoad(context) {
        ...
        if (context.type == context.UserEventType.VIEW) {
            ...
            let get_status = newRec.getText("status")
            let get_intercompany = newRec.getValue("intercotransaction")
            let get_related = getRelated(newRec.id)


            if (!get_related.includes("Item Fulfillment") && !get_related.includes("Invoice") && !get_intercompany && get_status!="Pending Approval" && !get_status.includes("Closed")) {
                ...
                context.form.addButton({
                    ...
                    label: "Transform Fulfillment & Invoice",
                    functionName: "onButtonClick(" + paramJson + ")"
                })
                try {
                    log.debug('client script fileid')
                    context.form.clientScriptModulePath = 'SuiteScripts/Metrodata/transform_so_to_if_inv/me_cs_transform_so_to_if_inv_button.js'
                } catch (error) {
