...
define(["N/record", "N/format", "N/search", "./config/me_config.js", './library/moment.min.js'], function (record, format, search, meconf, moment) {
  ...
  const TYPE = "customtransaction_me_pemenang_arisan_cst";
  ...
  function createPemenangArisan(context) {
    try {
  ...
  function updatePemenangArisan(context) {
    try {
  ...
  function inactivePemenangArisan(context) {
    try {
      var rec_cust = record.submitFields({
        type: TYPE,
        id: context.id,
  ...
  function post(context) {
    try {
      ...
      if (type == 'create') {
        var create = createPemenangArisan(context)
        return create
      }
      if (type == 'update') {
        var update = updatePemenangArisan(context)
        return update
      }
      if (type == 'inactive') {
        var inactive = inactivePemenangArisan(context)
        return inactive
      }
