...
define(["N/record", "N/format", "N/search", "./config/me_config.js", './library/moment.min.js'], function (record, format, search, meconf, moment) {
    ...
    const TYPE = "customrecord_me_csrec_multicur_tran_fund";
    ...
    function createTransferFund(context) {
      try {
    ...
    function updateTransferFund(context) {
      try {
    ...
    function inactiveTransferFund(context) {
      try {
        var rec_cust = record.submitFields({
          type: TYPE,
          id: context.id,
    ...
    function post(context) {
      try {
        ...
        if (type == 'create') {
          var create = createTransferFund(context)
          return create
        }
        if (type == 'update') {
          var update = updateTransferFund(context)
          return update
        }
        if (type == 'inactive') {
          var inactive = inactiveTransferFund(context)
          return inactive
        }
