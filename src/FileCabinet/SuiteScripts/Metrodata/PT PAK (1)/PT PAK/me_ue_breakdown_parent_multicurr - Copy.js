...
define(['N/record', 'N/search'], function (record, search) {
    ...
    function execute(context) {
        ...
        var meChildSearch = search.create({
            ...
            columns:
                [
                    ...
                    search.createColumn({
                        name: "internalid",
                        join: "CUSTRECORD_ME_GEMBD_NUMBER",
        ...
        for (let x = 0; x < meChildSearch.length; x++) {
            ...
            var currencyId = meChildSearch[x].getText(meChildSearch[x].columns[4]);

            for (let x = 0; x < result.length; x++) {
                if (result[x].item_id == itemId && result[x].currency_id == currencyId) {
                    countDup++;
                }
            }
