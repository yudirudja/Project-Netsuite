...
define(["N/record", "N/format", "N/search", "./config/me_config.js", './library/moment.min.js'], function (record, format, search, meconf, moment) {
    ...
    function createInventoryTransfer(context) {
      try {
    ...
    function updateInventoryTransfer(context) {
      try {
    ...
    function inactiveInventoryTransfer(context) {
      try {
        var rec_cust = record.submitFields({
          type: TYPE,
          id: context.id,
    ...
    function post(context) {
      try {
        ...
        if (type == 'create') {
          var create = createInventoryTransfer(context)
          return create
        }
        if (type == 'update') {
          var update = updateInventoryTransfer(context)
          return update
        }
        if (type == 'inactive') {
          var inactive = inactiveInventoryTransfer(context)
          return inactive
        }
