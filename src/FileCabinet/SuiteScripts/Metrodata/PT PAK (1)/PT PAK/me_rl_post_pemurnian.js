...
define(["N/record", "N/format", "N/search", "./config/me_config.js", './library/moment.min.js'], function (record, format, search, meconf, moment) {
  ...
  const TYPE = "customrecord_me_pemurnian_trx_pr";
      ...
      for (var fieldname in context) {
        if (context.hasOwnProperty(fieldname)) {
          ...
          if (!isSublist || !isObject) { //field biasa karena bukan sublist maupun object
            ...
            // }
            if (fieldname == "custrecord_me_pemurnian_finish_date") {
            ...
            } else if (fieldname != "custrecord_me_pemurnian_finish_date") {
  ...
  function updatePeleburan(context) {
    try {
      ...
      for (var fieldname in context) {
        ...
        // var subListId = 0
        if (fieldname == "recmachcustrecord_me_pemurnian_refinery_number_edit") {
          ...
          for (let x = 0; x < subArr.length; x++) {
            ...
            for (const fieldNameLine in subObj) {
              if (fieldNameLine == "customrecord_me_pemurnian_trx_chld_id") {
              ...
              // }
              if (fieldNameLine == "custrecord_me_pemurnian_bahan_metal") {
                bahanMetal = subObj[fieldNameLine];
              }
              if (fieldNameLine == "custrecord_me_pemurnian_berat") {
                beratMetal = subObj[fieldNameLine];
              }
              if (fieldNameLine == "custrecord_me_pemurnian_retur") {
                returMetal = subObj[fieldNameLine];
              }
              if (fieldNameLine == "custrecord_me_inventory_adjustment_trx") {
                invenAdjTrans = subObj[fieldNameLine];
              }
              record.submitFields({
                type: 'customrecord_me_pemurnian_trx_chld',
                id: subId,
                values: {
                  // custrecord_me_pencampuran_compound_num: compoundNumber,
                  custrecord_me_pemurnian_bahan_metal: bahanMetal,
                  custrecord_me_pemurnian_berat: beratMetal,
                  custrecord_me_pemurnian_retur: returMetal,
                  custrecord_me_inventory_adjustment_trx: invenAdjTrans,
                },
                options: {
