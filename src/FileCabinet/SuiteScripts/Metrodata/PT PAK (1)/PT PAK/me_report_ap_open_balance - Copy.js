    ...
    function (search, serverWidget, log, task, redirect, url, format, record, runtime, encode, render, file, moment) {
        ...
        function searchHutangTitip(data) {
            ...
            if (data.vendor != "") {
                filter.push(
                    "AND",
                    ["vendor.internalid", "anyof", data.vendor],);
            };
            ...
            do {
                ...
                for (let x = 0; x < result.length; x++) {
                    var vendorId = result[x].getValue(result[x].columns[0]);
                    var vendorName = result[x].getText(result[x].columns[0]);
                    ...
                    var amount = result[x].getValue(result[x].columns[13]);
                    ...
                    var subsidiaryId = result[x].getValue(result[x].columns[12]);
                    var applyingTransaction = result[x].getValue(result[x].columns[14]);
                    var applyingTransactionText = result[x].getText(result[x].columns[14]);
                    var applyingTransactionAmount = result[x].getValue(result[x].columns[15]);
                    var internalId = result[x].getValue(result[x].columns[16]);
                    var dateApplyingTrans = result[x].getValue(result[x].columns[17]);
                    var dueMonthTest = moment(dueDate, 'DD/M/YYYY').format('MMM YYYY');

                    log.debug("AMOUNT", amount + "_______" + currency + "_____" + subsidiary + "______" + type + "______" + applyingTransaction);
                    ...
                    for (let i = 0; i < dataSettlement.length; i++) {
                        if (dataSettlement[i].vendor_id == vendorId && dataSettlement[i].currency == currency && dataSettlement[i].subsidiary_id == subsidiaryId && dataSettlement[i].type_id == typeId && (moment(data.date, 'D/M/YYYY').diff(moment(dateApplyingTrans, 'D/M/YYYY'), 'days')) >= 0) {
            ...
            log.debug("hutang titip not obj123", rawReportData)
        ...
        function searchHutangDagang(data) {
            ...
            var transactionSearchObj = search.create({
                ...
                columns:
                    [
                        ...
                        search.createColumn({
                            name: "formulacurrency",
                            formula: "case when {applyingforeignamount} != 0 then {fxamount} - {applyingforeignamount} else {fxamount} end",
                            label: "Amount After payment/credit"
                        }),
                        //16
                        search.createColumn({
            ...
            do {
                ...
                for (let x = 0; x < result.length; x++) {
                    ...
                    for (let i = 0; i < reportData.length; i++) {
                        ...
                        //      }
                        //  }
                        if (reportData[i].internal_id != internalId && reportData[i].vendor_id == vendorId && reportData[i].currency == currency && reportData[i].subsidiary_id == subsidiaryId && reportData[i].type_id == typeId && ((applyingTransaction == "" || applyingTransactionAmount != 0))) {
                            reportData[i].amount += Number(((moment(data.date, 'D/M/YYYY').diff(moment(dateApplyingTrans, 'D/M/YYYY'), 'days')) >= 0 ? Number(amount) : Number(amountBeforeSettlement)));
                            if ((moment(dataMonth, 'MMM YYYY').diff(moment(dueMonthTest, 'MMM YYYY'), 'months')) >= 0) {
                                reportData[i].amount_before_curr_month += Number(((moment(data.date, 'D/M/YYYY').diff(moment(dateApplyingTrans, 'D/M/YYYY'), 'days')) >= 0 ? Number(amount) : Number(amountBeforeSettlement)));
                            }
                            if ((moment(dataMonth, 'MMM YYYY').diff(moment(dueMonthTest, 'MMM YYYY'), 'months')) < 0) {
                                reportData[i].amount_before_curr_month += Number(((moment(data.date, 'D/M/YYYY').diff(moment(dateApplyingTrans, 'D/M/YYYY'), 'days')) >= 0 ? Number(amount) : Number(amountBeforeSettlement)));
                            }
                    ...
                    if (countDup < 1 && (applyingTransaction == "" || applyingTransactionAmount != 0)) {
                        reportData.push({
                            vendor_id: vendorId,
                            vendor_name: vendorId,
                            company_name: companyName,
                            currency: currency,
                            currency_text: currencyText,
                            amount: Number(((moment(data.date, 'D/M/YYYY').diff(moment(dateApplyingTrans, 'D/M/YYYY'), 'days')) >= 0 ? Number(amount) : Number(amountBeforeSettlement))),
                            amount_before_curr_month: ((moment(dataMonth, 'MMM YYYY').diff(moment(dueMonthTest, 'MMM YYYY'), 'months')) >= 0 ? Number(((moment(data.date, 'D/M/YYYY').diff(moment(dateApplyingTrans, 'D/M/YYYY'), 'days')) >= 0 ? Number(amount) : Number(amountBeforeSettlement))) : 0),
                            amount_after_curr_month: ((moment(dataMonth, 'MMM YYYY').diff(moment(dueMonthTest, 'MMM YYYY'), 'months')) < 0 ? Number(((moment(data.date, 'D/M/YYYY').diff(moment(dateApplyingTrans, 'D/M/YYYY'), 'days')) >= 0 ? Number(amount) : Number(amountBeforeSettlement))) : 0),
                            period_post: periodPost,
                            subsidiary: subsidiary,
                            subsidiary_id: subsidiaryId,
                            type: type,
            ...
            log.debug("hutang Dagang", newReportData);
            return newReportData;
        }
