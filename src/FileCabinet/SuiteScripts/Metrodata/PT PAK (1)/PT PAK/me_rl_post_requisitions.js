...
define(["N/record", "N/format", "N/search", "./config/me_config.js", './library/moment.min.js'], function (record, format, search, meconf, moment) {
  ...
  function createRequisition(context) {
    try {
  ...
  function updateRequisition(context) {
    try {
  ...
  function inactiveRequisition(context) {
    try {
      var rec_cust = record.submitFields({
        type: TYPE,
        id: context.id,
  ...
  function post(context) {
    try {
      ...
      if (type == 'create') {
        var create = createRequisition(context)
        return create
      }
      if (type == 'update') {
        var update = updateRequisition(context)
        return update
      }
      if (type == 'inactive') {
        var inactive = inactiveRequisition(context)
        return inactive
      }
