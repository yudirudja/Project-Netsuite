...
define(["N/record", "N/format", "N/search", "./config/me_config.js", './library/moment.min.js'], function (record, format, search, meconf, moment) {
  ...
  function createCreditMemo(context) {
    try {
  ...
  function updateCreditMemo(context) {
    try {
  ...
  function inactiveCreditMemo(context) {
    try {
      var rec_cust = record.submitFields({
        type: TYPE,
        id: context.id,
  ...
  function post(context) {
    try {
      ...
      if (type == 'create') {
        var create = createCreditMemo(context)
        return create
      }
      if (type == 'update') {
        var update = updateCreditMemo(context)
        return update
      }
      if (type == 'inactive') {
        var inactive = inactiveCreditMemo(context)
        return inactive
      }
