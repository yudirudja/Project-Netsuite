                                ...
                                for (let i = 0; i < sublistEditLength; i++) {
                                    ...
                                    if (rec.getSublistValue({ sublistId: 'item', fieldId: 'item', line: i }) == parentId[x]) {
                                        ...
                                        for (let x = 0; x < lineArr.length; x++) {
                                            var lineObj = lineArr[x];
                                            for (const fieldNameLine in lineObj) {
                                                if (fieldNameLine == "item") {
                                                    var subLineObj = lineObj[fieldNameLine];
                                                    for (let p = 0; p < subLineObj.length; p++) {
                                                        var arrayFieldSub = lineObj[fieldNameLine];
                                                        for (const fieldNameLineSub in arrayFieldSub) {
                                                            if (fieldNameLineSub == "invdetail") {
