...
define(["N/record", "N/format", "N/search", "./config/me_config.js", './library/moment.min.js'], function (record, format, search, meconf, moment) {
    ...
    function createPoToBill(context) {
        try {
    ...
    function updatePoToBill(context) {
        try {
            var rec = record.load({
                type: TYPE,
                id: context.id,
            ...
            var getApplyLineCount = rec.getLineCount("apply");

            for (let i = 0; i < getApplyLineCount; i++) {
                var setAllApplyFalse = rec.setSublistValue({
                    sublistId: "apply",
                    fieldId: "apply",
                    value: false,
                    line: i,
    ...
    function inactivePoToBill(context) {
        try {
    ...
    function post(context) {
        try {
            ...
            if (type == "create") {
                var create = createPoToBill(context)
                return create;
            }
            if (type == "update") {
                var update = updatePoToBill(context)
                return update;
            }
            if (type == "inactive") {
                var inactive = inactivePoToBill(context)
                return inactive;
            }
        } catch (ex) {
            return doError(ex.name, ex.message, ex.stack);
