...
define(["N/record", "N/format", "N/search", "./config/me_config.js", './library/moment.min.js'], function (record, format, search, meconf, moment) {
  ...
  function createInvoice(context) {
    try {
  ...
  function updateInvoice(context) {
    try {
  ...
  function inactiveInvoice(context) {
    try {
      var rec_cust = record.submitFields({
        type: TYPE,
        id: context.id,
  ...
  function post(context) {
    try {
      ...
      if (type == 'create') {
        var create = createInvoice(context)
        return create
      }
      if (type == 'update') {
        var update = updateInvoice(context)
        return update
      }
      if (type == 'inactive') {
        var inactive = inactiveInvoice(context)
        return inactive
      }
