...
define(["N/record", "N/format", "N/search", "./config/me_config.js", './library/moment.min.js'], function (record, format, search, meconf, moment) {
    ...
    function createTransferOrder(context) {
      ...
      try {
        if ('apply' in context && Array.isArray(context.apply)) {
          for (let i = 0; i < context.apply.length; i++) {
            var applyInvoice = context.apply[i];
            applyLength = context.apply.length;
          ...
          return doError(ERROR_CODE.MISSING_REQ_OBJECT, "'apply' is not allowed to be empty, and please use [{}] instead of {} in 'apply' child");
        }
        ...
        for (let x = 0; x < parentId.length; x++) {
          ...
          for (var fieldname in context) {
            ...
            if (context.hasOwnProperty(fieldname) && fieldname != "undepfunds" && fieldname != "department" && fieldname != "class") {
              var isSublist = sublistName.indexOf(fieldname) > -1;
              var isApply = (fieldname == "apply");
              ...
              if (!isSublist || !isObject) { //field biasa karena bukan sublist maupun object
                ...
                var sublistEditLength = rec.getLineCount({ sublistId: 'apply' });
                ...
                for (let x = 0; x < parentId.length; x++) {
                  ...
                  for (let i = 0; i < sublistEditLength; i++) {
                    log.debug('invoice internal id sublist',rec.getSublistValue({sublistId:'apply',fieldId: 'internalid', line: i}))
                    if (rec.getSublistValue({sublistId:'apply',fieldId: 'internalid', line: i}) == parentId[x]) {
