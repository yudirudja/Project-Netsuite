...
define(["N/record", "N/format", "N/search", "./config/me_config.js", './library/moment.min.js'], function (record, format, search, meconf, moment) {
    ...
    const TYPE = "customtransaction_me_trx_deposit_arisan";
    ...
    function createDepoArisan(context) {
        try {
    ...
    function updateDepoArisan(context) {
        try {
            ...
            for (var fieldname in context) {
                if (fieldname == "lineupdate") {
                    var lineUpdate = context[fieldname];
    ...
    function inactiveDepoArisan(context) {
        try {
            var rec_cust = record.submitFields({
                type: TYPE,
                id: context.id,
    ...
    function post(context) {
        try {
            ...
            if (type == 'create') {
                var create = createDepoArisan(context)
                return create
            }
            if (type == 'update') {
                var update = updateDepoArisan(context)
                return update
            }
            if (type == 'inactive') {
                var inactive = inactiveDepoArisan(context)
                return inactive
            }
