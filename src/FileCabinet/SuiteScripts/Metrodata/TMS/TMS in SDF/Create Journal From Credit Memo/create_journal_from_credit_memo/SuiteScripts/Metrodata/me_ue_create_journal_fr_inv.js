...
define(['N/record','N/search', 'N/runtime'], function (record, search, runtime) {
    ...
    function beforeSubmit(context) {
      if (runtime.executionContext === runtime.ContextType.SUITELET || runtime.executionContext === runtime.ContextType.WEBSERVICES || runtime.executionContext === runtime.ContextType.SCHEDULED || runtime.executionContext === runtime.ContextType.MAP_REDUCE) {
        ...
        let get_je_trans = rec.getValue('custbody_me_invoice_je')
    ...
    function afterSubmit(context) {
      if (runtime.executionContext === runtime.ContextType.SUITELET || runtime.executionContext === runtime.ContextType.WEBSERVICES || runtime.executionContext === runtime.ContextType.SCHEDULED || runtime.executionContext === runtime.ContextType.MAP_REDUCE) {
        ...
        try {
            ...
            if (!get_je_trans && !get_approval_status.includes("Pending Approval")) {
                let create_journal = record.create({
                    type: record.Type.JOURNAL_ENTRY,
                });

                let set_bill_payment = create_journal.setValue('custbody_me_invoice_ar_number', recNew.id);
                ...
                let set_je_trans = rec.setValue('custbody_me_invoice_je', save)

            }

            if (get_je_trans && get_approval_status.includes("Approved")) {
                let load_journal = record.load({
                    type: record.Type.JOURNAL_ENTRY,
                    id: get_je_trans,
                });

                let set_bill_payment = load_journal.setValue('custbody_me_invoice_ar_number', recNew.id);
