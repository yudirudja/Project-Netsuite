...
define(['N/ui/serverWidget', 'N/render', 'N/record', './config/me_config_yudi.js', 'N/file', './lib/moment.min.js', 'N/search', 'N/https'], function (serverWidget, render, record, config, file, moment, search, https) {
    ...
    function formatNumberZeroDec(number) {
        // Ensure the input is a number
        if (isNaN(number)) {
            return '0.00';
        }
    ...
    function onRequest(context) {
        ...
        var getAccountNumber2 = recordLoadInv.getValue('custbody_me_account_number_bank_2');

        var getTerms = recordLoadInv.getText('terms');
        if (getTerms.includes('LC_sight')) {
            isShowDueDate = false
        }
        var getCustomerId = recordLoadInv.getValue('entity')
        var lookupCustomerName = search.lookupFields({
            type: search.Type.CUSTOMER,
            id: getCustomerId,
            columns: ['altname']
        });
        var getCustomerName = lookupCustomerName.altname;

        var getBillingAddress = recordLoadInv.getText('custbody_me_billing_address').replace("\r\n\r\n", "<br/><br/>");
        ...
        var getItemNameCust = loadCust.getSublistValue({
            sublistId: 'addressbook',
            fieldId: 'addressbookaddress_text',
            line: 0,
        });
        addressLine.push(getItemNameCust);
        ...
        var getBankInvoice2 = recordLoadInv.getText('custbody_me_bank_invoice_2');
        var getBankInvoiceAddress = recordLoadInv.getValue('custbody_me_alamat_bank_1');
        var getBankInvoiceAddress2 = recordLoadInv.getValue('custbody_me_alamat_bank_2');
        ...
        for (let i = 0; i < getLine; i++) {
            ...
            var getItemId = recordLoadInv.getSublistValue({
                sublistId: 'item',
                fieldId: 'item',
                line: i,
            });
            ...
            var getItemName = lookupItemName.displayname;
                ...
                if (getCurrency.includes('IDR')) {
                    ...
                    if (getBoundedZone.includes('No')) {
                        ...
                        sublistArr.push({
                            ...
                            quantity: formatNumberZeroDec(getQuantity),
        ...
        var printData = {
            ...
            terms: getTerms,
            bank1: getTerms + "<br/>" + getBankInvoice + "<br/>" + getBankInvoiceAddress + "<br/> AC Number: " + getAccountNumber,
            bank2: getBankInvoice2 + "<br/>" + getBankInvoiceAddress2 + (getAccountNumber2?"<br/> AC Number: ":"") + getAccountNumber2,
            line: sublistArr,
            line_address: getCustomerName + " <br/><br/>" + getBillingAddress,
        }
