...
define(["N/record", "N/format", "N/search", "./config/me_config_yudi.js", './lib/moment.min.js'], function (record, format, search, meconf, moment) {
  ...
  function updateBill(context) {
    try {
      ...
      for (var fieldname in context) {
          ...
          for (let i = 0; i < getInventoryLine; i++) {
            rec.removeLine({
              sublistId: 'inventory',
              line: i,
            });
  ...
  function post(context) {
    try {
      ...
      if (type == 'create') {
        var create = createBill(context)
        return create
      }
      if (type == 'update') {
        var update = updateBill(context)
        return update
      }
      if (type == 'inactive') {
        var inactive = inactiveBill(context)
        return inactive
      }
