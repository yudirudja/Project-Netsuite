 ...
 */
define(["N/runtime", "N/record", "N/url", "N/currency", 'N/search', './lib/moment.min.js', 'N/task'], function (runtime, record, url, currency, search, moment, task) {
    ...
    function afterSubmit(context) {
        ...
        var getBuApprover = newRec.getValue('custbody_me_bu_approver')

        var currUser = runtime.getCurrentUser().id;
        var currRole = runtime.getCurrentUser().role;

        var getRequstor = newRec.getValue('entity')

      log.debug('currRole',currRole)
      log.debug('currentUser',getRequstor)

        if (currRole == 3 && !getBuApprover) {
            ...
            if (getCurrEmployeeApproverPurchaser) {
                ...
                try {
                    ...
                    var getCurrentDate = moment(new Date(), 'D/M/YYYY').add(7, 'hours');
