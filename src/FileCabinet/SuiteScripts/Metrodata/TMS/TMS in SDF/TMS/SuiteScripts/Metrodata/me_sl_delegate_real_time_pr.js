...
define(["N/redirect", "N/ui/serverWidget", "N/task", "N/search", './config/me_config_yudi.js', 'N/file', 'N/url', './lib/moment.min.js', 'N/encode', 'N/format', 'N/record'],
    function (redirect, serverWidget, task, search, config, file, url, moment, encode, format, record) {
        ...
        function parameters(context) {
            ...
            log.debug('param Data', paramData)
            ...
            var delegate_deptF = form.addField({
                id: 'custpage_delegate_dept',
                type: serverWidget.FieldType.TEXT,
                label: "Delegate Dept",
                container: 'delegation'
            }).updateDisplayType({
            ...
            delegate_deptF.defaultValue = paramData.delegator_dept;
        ...
        function onRequest(context) {
            ...
            if (context.request.method == 'GET') {
                ...
                var delegate_dept = req_param['custpage_delegate_dept'];
                ...
                try {
                    ...
                    if (param.delegate_dept == 'BU') {
                        loadTransRec.setValue('custbody_me_delegate_rt_bu', param.delegate_to);
                        loadTransRec.setValue('nextapprover', param.delegate_to);
                    }
                    if (param.delegate_dept == 'Purchaser') {
                        loadTransRec.setValue('custbody_me_purchaser_rt_bu', param.delegate_to);
                        loadTransRec.setValue('nextapprover', param.delegate_to);
                    }
                    if (param.delegate_dept == 'Director') {
                        loadTransRec.setValue('custbody_me_delegate_rt_director', param.delegate_to);
                        loadTransRec.setValue('nextapprover', param.delegate_to);
                    }
                    if (param.delegate_dept == 'Pres Director') {
                        loadTransRec.setValue('custbody_me_delegate_rt_presdir', param.delegate_to);
                        loadTransRec.setValue('nextapprover', param.delegate_to);
                    }
                    loadTransRec.save();
                } catch (error) {
