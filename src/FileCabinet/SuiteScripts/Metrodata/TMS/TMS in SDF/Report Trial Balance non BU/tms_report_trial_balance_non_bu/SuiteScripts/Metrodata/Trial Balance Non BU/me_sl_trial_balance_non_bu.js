...
define(['N/ui/serverWidget', 'N/record', 'N/search', 'N/redirect', 'N/encode', 'N/file', '../lib/moment.min.js'], function (serverWidget, record, search, redirect, encode, file, moment) {
    ...
    function balanceSheetNonClosingSearch(params) {
        ...
        do {
            ...
            for (let i = 0; i < getbalanceSheet.length; i++) {
                var account_number = getbalanceSheet[i].getValue(getbalanceSheet[i].columns[4]);
                ...
                var name = getbalanceSheet[i].getValue(getbalanceSheet[i].columns[1]);
                var debit = getbalanceSheet[i].getValue(getbalanceSheet[i].columns[2]);
                var credit = getbalanceSheet[i].getValue(getbalanceSheet[i].columns[3]);
    ...
    function balanceSheetClosingSearch(params) {
            ...
            columns:
                [
                    ...
                    search.createColumn({
                        name: "formulanumeric",
                        summary: "SUM",
                        formula: "case when {custbody_me_post_period} like 'Adjust%' and {trandate} >= TO_DATE(" + "'" + params.start + "'" + ", 'D/M/YYYY') AND {trandate} <= TO_DATE(" + "'" + params.end + "'" + ", 'D/M/YYYY') and {debitamount} is not null and {account.type} like '%Accounts Payable%' then ({creditamount}*-1) when {trandate} >= TO_DATE(" + "'" + params.start + "'" + ", 'D/M/YYYY') AND {trandate} <= TO_DATE(" + "'" + params.end + "'" + ", 'D/M/YYYY') and {debitamount} is not null and {account.type} like '%Income%' then ({creditamount}) when {custbody_me_post_period} like 'Adjust%' and {trandate} >= TO_DATE(" + "'" + params.start + "'" + ", 'D/M/YYYY') AND {trandate} <= TO_DATE(" + "'" + params.end + "'" + ", 'D/M/YYYY') and {debitamount} is not null and {account.type} like '%Other Income%' then ({creditamount}) when {custbody_me_post_period} like 'Adjust%' and {trandate} >= TO_DATE(" + "'" + params.start + "'" + ", 'D/M/YYYY') AND {trandate} <= TO_DATE(" + "'" + params.end + "'" + ", 'D/M/YYYY') and {debitamount} is not null and {account.type} like '%Long Term Liability%' then ({creditamount}*-1) when {custbody_me_post_period} like 'Adjust%' and {trandate} >= TO_DATE(" + "'" + params.start + "'" + ", 'D/M/YYYY') AND {trandate} <= TO_DATE(" + "'" + params.end + "'" + ", 'D/M/YYYY') and {debitamount} is not null and {account.type} like '%Other Current Liability%' then ({creditamount}*-1) when {custbody_me_post_period} like 'Adjust%' and {trandate} >= TO_DATE(" + "'" + params.start + "'" + ", 'D/M/YYYY') AND {trandate} <= TO_DATE(" + "'" + params.end + "'" + ", 'D/M/YYYY') and {debitamount} is not null and {account.type} not like '%Accounts Payable%' then ({creditamount}*-1) when {custbody_me_post_period} like 'Adjust%' and {trandate} >= TO_DATE(" + "'" + params.start + "'" + ", 'D/M/YYYY') AND {trandate} <= TO_DATE(" + "'" + params.end + "'" + ", 'D/M/YYYY') and {debitamount} is not null and {account.type} not like '%Income%' then ({creditamount}*-1) when {custbody_me_post_period} like 'Adjust%' and {trandate} >= TO_DATE(" + "'" + params.start + "'" + ", 'D/M/YYYY') AND {trandate} <= TO_DATE(" + "'" + params.end + "'" + ", 'D/M/YYYY') and {debitamount} is not null and {account.type} not like '%Other Income%' then ({creditamount}*-1) when {custbody_me_post_period} like 'Adjust%' and {trandate} >= TO_DATE(" + "'" + params.start + "'" + ", 'D/M/YYYY') AND {trandate} <= TO_DATE(" + "'" + params.end + "'" + ", 'D/M/YYYY') and {debitamount} is not null and {account.type} like '%Long Term Liability%' then ({creditamount}*-1) when {custbody_me_post_period} like 'Adjust%' and {trandate} >= TO_DATE(" + "'" + params.start + "'" + ", 'D/M/YYYY') AND {trandate} <= TO_DATE(" + "'" + params.end + "'" + ", 'D/M/YYYY') and {debitamount} is not null and {account.type} not like '%Other Current Liability%' then ({creditamount}*-1)else 0 end",
                        label: "Credit"
                    }),
                ]
        });
        ...
        do {
            ...
            for (let i = 0; i < getbalanceSheet.length; i++) {
                var account_number = getbalanceSheet[i].getValue(getbalanceSheet[i].columns[4]);
                // var business_unit = getbalanceSheet[i].getValue(getbalanceSheet[i].columns[1]);
                var name_with_businss_unit = getbalanceSheet[i].getValue(getbalanceSheet[i].columns[5]);
                var name = getbalanceSheet[i].getValue(getbalanceSheet[i].columns[1]);
                var debit = getbalanceSheet[i].getValue(getbalanceSheet[i].columns[2]);
                var credit = getbalanceSheet[i].getValue(getbalanceSheet[i].columns[3]);
                var account = getbalanceSheet[i].getValue(getbalanceSheet[i].columns[0]);
                var account_text = getbalanceSheet[i].getText(getbalanceSheet[i].columns[0]);
                // var bu_id = getbalanceSheet[i].getValue(getbalanceSheet[i].columns[6]);
                var closing_trans_debit = getbalanceSheet[i].getValue(getbalanceSheet[i].columns[6]);
                var closing_trans_credit = getbalanceSheet[i].getValue(getbalanceSheet[i].columns[7]);
    ...
    function retainedEarningSearch(params) {
        ...
        var transactionSearchObj = search.create({
            ...
            filters:
                [
                    ["accounttype", "anyof", "COGS", "Expense", "Income", "OthExpense", "OthIncome"],
                    "AND",
                    ["posting", "is", "T"],
                    "AND",
                    ["trandate", "before", "1/1/" + moment(params.start, 'D/M/YYYY').year()]
                ],
            columns:
                [
                    search.createColumn({
        ...
        do {
            ...
            for (let i = 0; i < getRetainedEarnings.length; i++) {
                let account_type = getRetainedEarnings[i].getValue(getRetainedEarnings[i].columns[0])
                let amount = getRetainedEarnings[i].getValue(getRetainedEarnings[i].columns[1])
                let business_unit = getRetainedEarnings[i].getValue(getRetainedEarnings[i].columns[2])


                let get_business_unit_index = retainedArr.findIndex((data) => data.business_unit == business_unit);
                if (!retainedArr.some((data) => data.business_unit === business_unit)) {
                    retainedArr.push({
                        business_unit: business_unit,
                        [account_type]: Number(amount),
                    })
                } else {
                    retainedArr[get_business_unit_index][account_type] = Number(amount);
                }

                // retainedArr[get_business_unit_index].total = (!amount?(account_type == "Income" ?amount:0):0) - (!amount?(account_type == "Cost of Goods Sold" ?amount:0):0) - (!amount?(account_type == "Expense" ?amount:0):0) + (!amount?(account_type == "Other Expense" ?amount:0):0) + (!amount?(account_type == "Other Income" ?amount:0):0)

                if (business_unit == "- None -") {
                    let get_bu_none = retainedArr.filter((data) => data.business_unit == "- None -")

                    let get_retained_index = retainedArr.findIndex(data => data.business_unit == "NONE")

                    retainedArr[get_retained_index][account_type] += get_bu_none[0][account_type]
                }
            }
            startRow += 1000
        } while (getRetainedEarnings.length === 1000);

        let total_each_account = {
            COGS: 0,
            Expense: 0,
            Income: 0,
            OthExpense: 0,
        ...
        let total_retained_previous = 0
        for (let i = 0; i < retainedArr.length; i++) {
            // if (retainedArr[i].business_unit == "- None -") {
            //     continue;
            // }
            // total_retained_previous += (retainedArr[i].Income ? retainedArr[i].Income : 0) - (retainedArr[i].COGS ? retainedArr[i].COGS : 0) - (retainedArr[i].Expense ? retainedArr[i].Expense : 0) + (retainedArr[i].OthIncome ? retainedArr[i].OthIncome : 0) - (retainedArr[i].OthExpense ? retainedArr[i].OthExpense : 0);
            total_each_account.Income += (retainedArr[i].Income ? retainedArr[i].Income : 0)
            total_each_account.COGS += (retainedArr[i].COGS ? retainedArr[i].COGS : 0)
            total_each_account.Expense += (retainedArr[i].Expense ? retainedArr[i].Expense : 0)
            total_each_account.OthIncome += (retainedArr[i].OthIncome ? retainedArr[i].OthIncome : 0)
            total_each_account.OthExpense += (retainedArr[i].OthExpense ? retainedArr[i].OthExpense : 0)
        }

        total_retained_previous = total_each_account.Income - total_each_account.COGS  - total_each_account.Expense + total_each_account.OthIncome - total_each_account.OthExpense;


        log.debug('total_retained_previous', total_retained_previous)
        log.debug('retainedArr', retainedArr)
        return total_retained_previous;
    }
    function retainedEarningSearchOld(params) {
    ...
    function balanceSheetOpeningSearch(params, filter) {
            ...
            columns:
                [
                    ...
                    search.createColumn({
                        name: "formulanumeric",
                        summary: "SUM",
                        formula: `case when {trandate} < TO_DATE('${params.start}', 'D/M/YYYY') and {trandate} > TO_DATE('1/1/${moment(params.start, 'D/M/YYYY').year()}', 'D/M/YYYY') and {debitamount} is not null and {account.type} like '%Income%' then ({debitamount}*-1) when {trandate} < TO_DATE('${params.start}', 'D/M/YYYY') and {trandate} > TO_DATE('1/1/${moment(params.start, 'D/M/YYYY').year()}', 'D/M/YYYY') and {debitamount} is not null and {account.type} like '%Other Income%' then ({debitamount}*-1) when {trandate} < TO_DATE('${params.start}', 'D/M/YYYY') and {trandate} > TO_DATE('1/1/${moment(params.start, 'D/M/YYYY').year()}', 'D/M/YYYY') and {debitamount} is not null and {account.type} like '%Expense%' then ({debitamount}) when {trandate} < TO_DATE('${params.start}', 'D/M/YYYY') and {trandate} > TO_DATE('1/1/${moment(params.start, 'D/M/YYYY').year()}', 'D/M/YYYY') and {debitamount} is not null and {account.type} like '%Other Expense%' then ({debitamount})when {trandate} < TO_DATE('${params.start}', 'D/M/YYYY') and {trandate} > TO_DATE('1/1/${moment(params.start, 'D/M/YYYY').year()}', 'D/M/YYYY') and {debitamount} is not null and {account.type} like '%Cost of Goods Sold%' then ({debitamount}) else 0 end`,
                        label: "Formula (Numeric)"
                    }),
                    search.createColumn({
                        name: "formulanumeric",
                        summary: "SUM",
                        formula: `case when {trandate} < TO_DATE('${params.start}', 'D/M/YYYY') and {trandate} > TO_DATE('1/1/${moment(params.start, 'D/M/YYYY').year()}', 'D/M/YYYY') and {creditamount} is not null and {account.type} like '%Income%' then ({creditamount}) when {trandate} < TO_DATE('${params.start}', 'D/M/YYYY') and {trandate} > TO_DATE('1/1/${moment(params.start, 'D/M/YYYY').year()}', 'D/M/YYYY') and {creditamount} is not null and {account.type} like '%Other Income%' then ({creditamount}) when {trandate} < TO_DATE('${params.start}', 'D/M/YYYY') and {trandate} > TO_DATE('1/1/${moment(params.start, 'D/M/YYYY').year()}', 'D/M/YYYY') and {creditamount} is not null and {account.type} like '%Expense%' then ({creditamount}*-1) when {trandate} < TO_DATE('${params.start}', 'D/M/YYYY') and {trandate} > TO_DATE('1/1/${moment(params.start, 'D/M/YYYY').year()}', 'D/M/YYYY') and ({creditamount}) is not null and {account.type} like '%Other Expense%' then ({creditamount}*-1) when {trandate} < TO_DATE('${params.start}', 'D/M/YYYY') and {trandate} > TO_DATE('1/1/${moment(params.start, 'D/M/YYYY').year()}', 'D/M/YYYY') and ({creditamount}) is not null and {account.type} like '%Cost of Goods Sold%' then ({creditamount}*-1) else 0 end`,
                        label: "Formula (Numeric)"
                    }),
                    search.createColumn({
                        name: "accounttype",
        ...
        do {
            ...
            for (let i = 0; i < getbalanceSheet.length; i++) {
                ...
                var account_text = getbalanceSheet[i].getText(getbalanceSheet[i].columns[0]);
                // var bu_id = getbalanceSheet[i].getValue(getbalanceSheet[i].columns[6]);
                var debit_income_statement = getbalanceSheet[i].getValue(getbalanceSheet[i].columns[6]);
                var credit_income_statement = getbalanceSheet[i].getValue(getbalanceSheet[i].columns[7]);
                var account_type = getbalanceSheet[i].getValue(getbalanceSheet[i].columns[8]);

                result.push({
                    account_number: account_number,
                    // business_unit: business_unit,
                    name: name_with_businss_unit,
                    debit: account_type == "COGS" || account_type == "Expense" || account_type == "Income" || account_type == "OthExpense" || account_type == "OthIncome" ? parseFloat(debit_income_statement) : parseFloat(debit),
                    credit: account_type == "COGS" || account_type == "Expense" || account_type == "Income" || account_type == "OthExpense" || account_type == "OthIncome" ? parseFloat(credit_income_statement) : parseFloat(credit),
                    opening_blnc: account_type == "COGS" || account_type == "Expense" || account_type == "Income" || account_type == "OthExpense" || account_type == "OthIncome" ? parseFloat(debit_income_statement) + parseFloat(credit_income_statement) : parseFloat(debit) + parseFloat(credit),
                    account: account,
                    account_text: account_text,
                    account_text_parent: account_text.split(':')[0],
                    // bu_id: bu_id,
    ...
    function printHtmlNonClosing(form, final, context, param, get_retained_earning) {
        ...
        for (let p = 0; p < final.length; p++) {
            ...
            if (!(final[p].name).includes('RETAINED EARNINGS')) {
                ...
                // let get_retained_previous_year = get_retained_earning.filter((data) => data.account_number === final[p].account_number);
                if (get_retained_earning > 0) {
                    log.debug("get_retained_earning NON CLOSING", get_retained_earning)
                    // let total_retained_previous = (get_retained_previous_year[0].Income ? get_retained_previous_year[0].Income : 0) - (get_retained_previous_year[0].COGS ? get_retained_previous_year[0].COGS : 0) - (get_retained_previous_year[0].Expense ? get_retained_previous_year[0].Expense : 0) + (get_retained_previous_year[0].OthIncome ? get_retained_previous_year[0].OthIncome : 0) - (get_retained_previous_year[0].OthExpense ? get_retained_previous_year[0].OthExpense : 0);
                    ...
                    salesReport.defaultValue +=
                        ...
                        // '<td >' + ((final[p].business_unit).includes('- None -') || (final[p].business_unit).includes('None') ? '' : final[p].business_unit) + '</td>' +
                        '<td >' + final[p].name + '</td>' +
                        '<td >' + addComa(Number(final[p].opening_blnc) - get_retained_earning) + '</td>' +
                        '<td >' + addComa(Math.abs(final[p].debit)) + '</td>' +
                        '<td >' + addComa((Math.abs(final[p].credit))) + '</td>' +
                        '<td >' + addComa(Number(final[p].opening_blnc) + Number(final[p].debit) + Number(final[p].credit) - get_retained_earning) + '</td>';
                    salesReport.defaultValue += '</tr>';
                } else {
                    salesReport.defaultValue +=
                        '<tr >' +
    ...
    function printHtmlClosing(form, final, context, param, get_retained_earning) {
        ...
        for (let p = 0; p < final.length; p++) {
            ...
            if (!(final[p].name).includes('RETAINED EARNINGS')) {
                ...
                // let get_retained_previous_year = get_retained_earning.filter((data) => data.account_number === final[p].account_number);
                // log.debug("get_retained_previous_year",get_retained_previous_year)


                log.debug("get_retained_earning", get_retained_earning)
                if (get_retained_earning > 0) {
                    // let total_retained_previous = (get_retained_previous_year[0].Income ? get_retained_previous_year[0].Income : 0) - (get_retained_previous_year[0].COGS ? get_retained_previous_year[0].COGS : 0) - (get_retained_previous_year[0].Expense ? get_retained_previous_year[0].Expense : 0) + (get_retained_previous_year[0].OthIncome ? get_retained_previous_year[0].OthIncome : 0) - (get_retained_previous_year[0].OthExpense ? get_retained_previous_year[0].OthExpense : 0);
                    ...
                    salesReport.defaultValue +=
                        ...
                        '<td >' + addComa((Math.abs(final[p].credit))) + '</td>' +
                        '<td >' + addComa(Number(final[p].closing_trans)) + '</td>' +
                        '<td >' + addComa(Number(final[p].opening_blnc) + Number(final[p].debit) + Number(final[p].credit) - get_retained_earning + Number(final[p].closing_trans)) + '</td>';
                    salesReport.defaultValue += '</tr>';
                } else {
                    salesReport.defaultValue +=
                        '<tr >' +
    ...
    function parameters() {
        var form = serverWidget.createForm({ title: 'Trial Balance Report Non BU' });
    ...
    function onRequest(context) {
        ...
        if (context.request.method === 'GET') {
            ...
            if (category == 1) {
                var get_balance_sheet = balanceSheetNonClosingSearch(param)
            ...
            if (category == 2) {
                var get_balance_sheet = balanceSheetClosingSearch(param)
                var get_retained_earning = retainedEarningSearch(param)
                var get_balance_sheet_account = getBalanceAccountBU(get_balance_sheet)
                var get_balance_sheet_opening = balanceSheetOpeningSearch(param, get_balance_sheet_account)
                var combine_saved_search = combine(get_balance_sheet, get_balance_sheet_opening)
