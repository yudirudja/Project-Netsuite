function customizeGlImpact(transactionRecord, standardLines, customLines, book) {
    try {
        ...
        var created_from = transactionRecord.getFieldValue('custbody_me_related_po_bill');
        ...
        for (var i = 1; i <= curr_line_count; i++) {
            ...
            for (var j = 0; j < item_po_array.length; j++) {
                if (item_po_array[j].get_item_id == get_curr_item_id && item_account != '' && (get_generate_accrual_item == 'F' || !get_generate_accrual_item)) {
                    cust_gl_val.push({
        ...
        for (var i = 0; i < cust_gl_val.length; i++) {
            // if (cust_gl_val[i].account_id == 54) {
                ...
                newLine.setMemo("Journal Balik Otomatis dibuatkan oleh Metrodata untuk Item yang General Accruals = No");
