            ...
            columns:
               [
                  ...
                  search.createColumn({
                     ...
                     formula: "case when {currency} like '%IDR%' then {custcol_me_lme_final} * {custcol_me_kurs_kesepakatan} when {currency} like '%US%' then {custcol_me_lme_final} end",
                     label: "LME"
                  }),
                  search.createColumn({
                     name: "formulacurrency",
                     formula: "case when {currency} like '%IDR%' then {custcol_me_premium_final} * {custcol_me_kurs_kesepakatan} when {currency} like '%US%' then {custcol_me_premium_final} end",
                     label: "Premium"
                  }),
                  search.createColumn({
                     name: "formulacurrency",
                     formula: "case when {currency} like '%IDR%' then {custcol_me_mjp_price} * {custcol_me_kurs_kesepakatan} when {currency} like '%US%' then {custcol_me_mjp_price} end",
                     label: "MJP"
                  }),
                  search.createColumn({
                     name: "formulacurrency",
                     formula: "case when {currency} like '%IDR%' then {custcol_me_other_final} * {custcol_me_kurs_kesepakatan} when {currency} like '%US%' then {custcol_me_other_final} end",
                     label: "Other"
                  }),
                  search.createColumn({
                     name: "formulacurrency",
                     formula: "case when {currency} like '%IDR%' then ({custcol_me_lme_final} + {custcol_me_premium_final} + {custcol_me_mjp_price} + {custcol_me_other_final})*{custcol_me_kurs_kesepakatan} when {currency} like '%US%' then ({custcol_me_lme_final} + {custcol_me_premium_final} + {custcol_me_mjp_price} + {custcol_me_other_final}) end",
                     label: "Unit Price"
                  }),
      ...
      function combine(salesRep, inv) {
         for (let i = 0; i < salesRep.length; i++) {
            ...
            for (let j = 0; j < getDuplicateInvoiceNitem.length; j++) {
               ...
               salesRep[i].do.push(getDuplicateInvoiceNitem[j])
      ...
      function onRequest(context) {
         ...
         if (context.request.method == 'GET') {
            ...
            if (invIdArr.length > 0) {
               ...
               if (print_as === '1') {
                  var printReportHtml = printHtml(form, combineArr, context);
               } else if (print_as === '2') {
