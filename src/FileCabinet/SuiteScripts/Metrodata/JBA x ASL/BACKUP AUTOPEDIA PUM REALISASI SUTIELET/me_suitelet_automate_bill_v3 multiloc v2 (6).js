        ...
        function onRequest(context) {
            ...
            if (context.request.method === 'GET') {
                ...
                if (parameter_data.rec_type === 'purchase_order') {
                    ...
                    let get_nomor_doc = getNomorDocPo(get_pum_data, parameter_data);
                ...
                for (let i = 0; i < lineCount; i++) {
                    ...
                    if (isCheckedHidden === 'T') {
                        ...
                        if (checked_array.some((data) => data.PumRealisasiId == PumRealisasiId)) {
                            let get_index = checked_array.findIndex((data) => data.PumRealisasiId == PumRealisasiId)
                            whtAmount ? checked_array[get_index].whtAmount += Number(whtAmount) : ""
                            whtAmount ? checked_array[get_index].amount_titipan_hidden = Number(whtAmount) - Math.abs(checked_array[get_index].amount_titipan_hidden) : ""
                            ...
                            checked_array.push({
                                ...
                                amount_titipan_hidden: Number(whtAmount) != 0 ? Number(whtAmount) - Math.abs(amount_titipan_hidden) : Math.abs(amount_titipan_hidden),
                ...
                for (let i = 0; i < checked_arr_new.length; i++) {
                    ...
                    if (checked_arr_new[i].amount_titipan_hidden != 0 || checked_arr_new[i].check_deposit_amount != 0) {//set Amount titipan/ remaining amount (Debit atau credit, tergantung amount titipan bernilai minus atau tidak)
                        ...
                        if (checked_arr_new[i].check_deposit_amount) {
                            let set_ammount_titipan = jeRecord.setCurrentSublistValue({
                                ...
                                value: Math.abs(checked_arr_new[i].check_deposit_amount)
                            });
                            ...
                            let set_ammount_titipan = jeRecord.setCurrentSublistValue({
                                ...
                                value: Math.abs(checked_arr_new[i].amount_titipan_hidden)
                            });
                    ...
                    //     jeRecord.commitLine('line');
                    // }
                    if (checked_arr_new[i].whtAmount != 0 && !isNaN(checked_arr_new[i].whtAmount)) { // new WHT INPUT
                        for (let j = 0; j < checked_arr_new[i].wht_list.length; j++) {

                            jeRecord.selectNewLine('line'); // set amount wht (credit)
                            let set_account_wht = jeRecord.setCurrentSublistValue({
                                sublistId: 'line',
                                fieldId: 'account',
                                value: checked_arr_new[i].wht_list[j].wht_account
                            });
                            let set_memo_wht = jeRecord.setCurrentSublistValue({
                                sublistId: 'line',
                                fieldId: 'memo',
                            ...
                            let set_ammount_wht = jeRecord.setCurrentSublistValue({
                                sublistId: 'line',
                                fieldId: 'credit',
                                value: checked_arr_new[i].wht_list[j].whtAmount
                            });
                            jeRecord.commitLine('line');
                        }
                    }
                ...
                form_filter_result.clientScriptModulePath = 'SuiteScripts/METRODATA/me_sl_cs_automate_bill_v3.js';
