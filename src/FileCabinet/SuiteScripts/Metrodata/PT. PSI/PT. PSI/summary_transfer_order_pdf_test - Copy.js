    ...
    function (search, render, file, record, encode, format, task, redirect, serverWidget) {
        ...
        function calculateAmountModTest(itemData, arrData, paramCalculate) {
            ...
            for (var j = 0; j < getDataArr.length; j++) {
                ...
                for (var k = j + 1; k < getDataArr.length; k++) {
                    ...
                    if (arrData[j].transfer_order.documentNumber == arrData[k].transfer_order.documentNumber ) {
                        arrData[j].data_item.amount_order = calculateAmtOrdInt;
                        arrData[j].transfer_order.amount_order_total = k == j+1? calculateAmtOrdTotalInt : (Math.abs(arrData[k].data_item.quantity) * (Math.abs(arrData[k].data_item.price) / Math.abs(arrData[k].data_item.quantity_conversion))) + prevCalculateAmtOrd;
                        currentAddition = k == j+1? calculateAmtOrdTotalInt : (Math.abs(arrData[k].data_item.quantity) * (Math.abs(arrData[k].data_item.price) / Math.abs(arrData[k].data_item.quantity_conversion))) + prevCalculateAmtOrd;
                        ...
                        saveIndex.push(k);
                        isAdded = false;
                        prevCalculateAmtOrd = currentAddition;
                ...
                // while (x < getDataArr.length) {
                //     if (arrData[x].transfer_order.documentNumber == arrData[l].transfer_order.documentNumber && x != l) {
                //         arrData[x].data_item.amount_order = calculateAmtOrdInt;
                //         arrData[x].transfer_order.amount_order_total = calculateAmtOrdTotalInt + prevCalculateAmtOrd;
                //         arrData[x].data_item.amount_recieved = calculateAmtReceiptInt;
                //         // arrData[j].transfer_order.amount_recieved_total = parseInt(calculateAmtReceiptTotalInt) + prevCalculateAmtReceiptTotalInt;
                //         arrData[x].data_item.cartonFullfillemntTotal = parseInt(convertQuantitytoCartonInt);
                //         arrData[x].data_item.pcsFullfillmentTotal = convertQuantitytoPcs;
                //         arrData[x].data_item.sisa_carton = parseInt(sisaCtnInt);
                //         arrData[x].data_item.sisa_pcs = sisaPcs;
                //         arrData[x].data_item.sisa_amount = sisaAmountItemNotRecievedInt;
                //         arrData[x].transfer_order.amount_sisa_total = sisaAmountItemNotRecievedTotalInt + prevSisaAmountItemNotRecievedTotalInt;
                //         arrData[x].data_item.amount_order = calculateAmtOrdInt;
                //         // arrData[j].data_item.amount_order = parseInt(calculateAmtOrdInt);
                //         arrData[x].data_receipt.amountRecievedEachReciept = calculateAmountEachReceiptInt;
                //     }
        ...
        function createPdfTransferOrder(context) {
            ...
            if (internalItemIdArr.length > 1) {
                ...
                //UBAH OUTPUT SISA PCS/CARTON DISINI
